...
export async function overlayImages(req, res) {
  ...
  let sneakerImg = await sharp("./" + sneakerUrl)
    ...
    .toBuffer();
  let designImg = "./images/design-resized.png"; // if (designUrl) {
  ...
  designImg = await generateDesignImage(req, res); // get dimensions

  let metadata = {};
  metadata.mainImg = await mainImg.metadata();
  metadata.logoImg = sizeOf(logoImg);
  metadata.sneakerImg = sizeOf(sneakerImg); // console.log(metadata);
  // get output image

  let currentData = Date.now();
  let outputImg = await sharp("./" + mainUrl) // .resize({ width: 400 })
    .composite([
      {
        input: logoImg,
        gravity: "southeast",
...
export async function getAnImageLocally(req, res) {
  ...
  let t1 = performance.now(); // console.log("Time (ms) to fetch an Image by an API = ", t1 - t0);
...
export async function getAnImageFromApi(req, res) {
  ...
  let t1 = performance.now(); // console.log(
...
async function generateDesignImage(req, res) {
  ...
  const decodedData = data ? JSON.parse(data) : []; //process to edit and get the image file
