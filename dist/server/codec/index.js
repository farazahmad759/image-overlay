...
import { createSVGWindow } from "svgdom"; //import Debug from "debug";
...
const {
  document
} = window; //const debug = Debug("convert");
//match the property
...
async function modifySVG(rootCanvas, id, property, value) {
  if (property === "background-colour") {
    ...
    element.map(inner => {
      if (inner.type === "g") {
        let hexValue = "#" + value; //its a group, fill the children
    ...
    const toBase64 = fs.readFileSync(backgroundImage, "base64"); //svg processing
    ...
    try {
      ...
      let childrenPath = box.find("path"); //check the children length
...
const processImage = async (file, spec) => {
  registerWindow(window, document); //get the design that was requested
  ...
  if (rootCanvas.type === "svg") {
    ...
    spec.map(obj => {
      ...
      if (id && property && value) {
        let _lowercase = property.toLowerCase(); //make changes to the svg
